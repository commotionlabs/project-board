...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="sm:hidden fixed bottom-0 inset-x-0 z-30 border-t bg-background/95 backdrop-blur">
        <div className="grid grid-cols-4 gap-1 p-2">
          <Button variant={view === 'kanban' ? 'default' : 'outline'} className="h-10" onClick={() => setView('kanban')}><LayoutGrid className="h-4 w-4" /></Button>
          <Button variant={view === 'list' ? 'default' : 'outline'} className="h-10" onClick={() => setView('list')}><List className="h-4 w-4" /></Button>
          <Button variant="outline" className="h-10" onClick={() => setPaletteOpen(true)}><Search className="h-4 w-4" /></Button>
          <Button className="h-10" onClick={() => handleAddTask('todo')}><Plus className="h-4 w-4" /></Button>
        </div>
      </div>
